...
void ampabcdPage_selection() 
{
  ...
  if (auditionMode == 1)  // On auditionMode there is tunerscreen change on screen 12 and Flash change screen 14
  {
  ...
  if (strcmp(page, "preset") != 0)  // -->>  NOT on presetpage
        { 
          ...
          if (PresetNumb < 0) 
          {
            ...
            if (PresetNumb > maxPreset) {PresetNumb = (PresetNumb - 10);}  // 511 is last passible number}
            } 
